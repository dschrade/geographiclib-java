 ...
 * <i>azi1</i> are specified in the constructor; alternatively, the
 * {@link Geodesic#Line Geodesic.Line} method can be used to create a
 * GeodesicLine. {@link #Position Position} returns the location of point 2 a
 * distance <i>s12</i> along the geodesic. Alternatively {@link #ArcPosition
 * ArcPosition} gives the position of point 2 an arc length <i>a12</i> along the
 * geodesic.
 * <p>
 * You can register the position of a reference point 3 a distance (arc length),
 * <i>s13</i> (<i>a13</i>) along the geodesic with the {@link #SetDistance
 * SetDistance} ({@link #SetArc SetArc}) functions. Points a fractional distance
 * along the line can be found by providing, for example, 0.5 *
 * {@link #Distance} as an argument to {@link #Position Position}. The
 ...
 * NaNs.
 * <p>
 * The calculations are accurate to better than 15 nm (15 nanometers). See Sec.
 * 9 of <a href="https://arxiv.org/abs/1102.1215v1">arXiv:1102.1215v1</a> for
 ...
 * (<a href=
 * "https://geographiclib.sourceforge.io/geod-addenda.html">addenda</a>).
 * </ul>
 * <p>
 ...
 *         int num = (int) (StrictMath.ceil(line.Distance() / ds0));
 *         {
...
public class GeodesicLine {
    ...
    private double _a, _f, _b, _c2, _f1, _salp0, _calp0, _k2, _salp1, _calp1, _ssig1, _csig1, _dn1, _stau1, _ctau1,
            _somg1, _comg1, _A1m1, _A2m1, _A3c, _B11, _B21, _B31, _A4, _B41;
     ...
     * 
     * @param g
     *            A {@link Geodesic} object used to compute the necessary
     ...
     *            <i>lon1</i> fixed, writing <i>lat1</i> = &plusmn;(90&deg; &minus;
     ...
     *            quantities can be returned in calls to {@link #Position Position}.
     *            <p>
    ...
    private void LineInit(Geodesic g, double lat1, double lon1, double azi1, double salp1, double calp1, int caps,
        ...
        sbet1 = p.first;
        cbet1 = StrictMath.max(Geodesic.tiny_, p.second);
        _dn1 = StrictMath.sqrt(1 + g._ep2 * GeoMath.sq(sbet1));

        // Evaluate alp0 from sin(alp1) * cos(bet1) = sin(alp0),
        _salp0 = _salp1 * cbet1; // alp0 in [0, pi/2 - |bet1|]
        // Alt: calp0 = StrictMath.hypot(sbet1, calp1 * cbet1). The following
        // is slightly better (consider the case salp1 = 0).
        _calp0 = StrictMath.hypot(_calp1, _salp1 * sbet1);
        ...
        double eps = _k2 / (2 * (1 + StrictMath.sqrt(1 + _k2)) + _k2);
        ...
        if ((_caps & GeodesicMask.CAP_C1) != 0) {
            ...
            double s = StrictMath.sin(_B11), c = StrictMath.cos(_B11);
            // tau1 = sig1 + B11
    ...
    protected GeodesicLine(Geodesic g, double lat1, double lon1, double azi1, double salp1, double calp1, int caps,
            boolean arcmode, double s13_a13) {
        Pair p = new Pair();
        LineInit(g, lat1, lon1, azi1, salp1, calp1, caps, p);
    ...
    /**
     * A default constructor. If GeodesicLine.Position is called on the resulting
     * object, it returns immediately (without doing any calculations). The object
     * can be set with a call to {@link Geodesic.Line}. Use {@link Init()} to test
     * whether object is still in this uninitialized state. (This constructor was
     * useful in C++, e.g., to allow vectors of GeodesicLine objects. It may not be
     * needed in Java, so make it private.)
     **********************************************************************/
    ...
    /**
     * Compute the position of point 2 which is a distance <i>s12</i> (meters) from
     * point 1.
     * <p>
     * 
     * @param s12
     *            distance from point 1 to point 2 (meters); it can be negative.
     * @return a {@link GeodesicData} object with the following fields: <i>lat1</i>,
     *         <i>lon1</i>, <i>azi1</i>, <i>lat2</i>, <i>lon2</i>, <i>azi2</i>,
     *         <i>s12</i>, <i>a12</i>. Some of these results may be missing if the
     *         GeodesicLine did not include the relevant capability.
     *         <p>
     *         The values of <i>lon2</i> and <i>azi2</i> returned are in the range
     *         [&minus;180&deg;, 180&deg;].
     *         <p>
     *         The GeodesicLine object <i>must</i> have been constructed with
     ...
     *         The GeodesicLine object <i>must</i> have been constructed with
     *         <i>caps</i> |= {@link GeodesicMask#DISTANCE_IN}; otherwise no
     *         parameters are set. Requesting a value which the GeodesicLine object
     *         is not capable of computing is not an error (no parameters will be
     *         set). The value of <i>lon2</i> returned is normally in the range
     *         [&minus;180&deg;, 180&deg;]; however if the <i>outmask</i> includes
     *         the {@link GeodesicMask#LONG_UNROLL} flag, the longitude is
     *         "unrolled" so that the quantity <i>lon2</i> &minus; <i>lon1</i>
    ...
    /**
     * Compute the position of point 2 which is an arc length <i>a12</i> (degrees)
     * from point 1.
     * <p>
     * 
     * @param a12
     *            arc length from point 1 to point 2 (degrees); it can be negative.
     * @return a {@link GeodesicData} object with the following fields: <i>lat1</i>,
     *         <i>lon1</i>, <i>azi1</i>, <i>lat2</i>, <i>lon2</i>, <i>azi2</i>,
     *         <i>s12</i>, <i>a12</i>. Some of these results may be missing if the
     *         GeodesicLine did not include the relevant capability.
     ...
     *         <i>lon2</i> returned is in the range [&minus;180&deg;, 180&deg;],
     *         unless the <i>outmask</i> includes the
    ...
    /**
     * The general position function. {@link #Position(double, int) Position} and
     * {@link #ArcPosition(double, int) ArcPosition} are defined in terms of this
     * function.
     * <p>
     * 
     * @param arcmode
     *            boolean flag determining the meaning of the second parameter; if
     *            arcmode is false, then the GeodesicLine object must have been
     *            constructed with <i>caps</i> |= {@link GeodesicMask#DISTANCE_IN}.
     * @param s12_a12
     ...
     *         <li><i>outmask</i> |= {@link GeodesicMask#LONG_UNROLL} to unroll
     *         <i>lon2</i> (instead of reducing it to the range [&minus;180&deg;,
     *         180&deg;]).
     *         </ul>
     *         <p>
     *         Requesting a value which the GeodesicLine object is not capable of
    ...
    public GeodesicData Position(boolean arcmode, double s12_a12, int outmask) {
        ...
        if (arcmode) {
            ...
            r.a12 = s12_a12;
            sig12 = CustomStrictMath.toRadians(s12_a12);
            Pair p = new Pair();
            ...
            r.s12 = s12_a12;
            double tau12 = s12_a12 / (_b * (1 + _A1m1)), s = StrictMath.sin(tau12), c = StrictMath.cos(tau12);
            // tau2 = tau1 + tau12
            B12 = -Geodesic.SinCosSeries(true, _stau1 * c + _ctau1 * s, _ctau1 * c - _stau1 * s, _C1pa);
            sig12 = tau12 - (B12 - _B11);
            ssig12 = StrictMath.sin(sig12);
            csig12 = StrictMath.cos(sig12);
            if (StrictMath.abs(_f) > 0.01) {
                ...
                sig12 = sig12 - serr / StrictMath.sqrt(1 + _k2 * GeoMath.sq(ssig2));
                ssig12 = StrictMath.sin(sig12);
                csig12 = StrictMath.cos(sig12);
                // Update B12 below
            }
            r.a12 = CustomStrictMath.toDegrees(sig12);
        }
        ...
        double dn2 = StrictMath.sqrt(1 + _k2 * GeoMath.sq(ssig2));
        if ((outmask & (GeodesicMask.DISTANCE | GeodesicMask.REDUCEDLENGTH | GeodesicMask.GEODESICSCALE)) != 0) {
            if (arcmode || StrictMath.abs(_f) > 0.01)
                B12 = Geodesic.SinCosSeries(true, ssig2, csig2, _C1a);
            AB1 = (1 + _A1m1) * (B12 - _B11);
        }
        // sin(bet2) = cos(alp0) * sin(sig2)
        sbet2 = _calp0 * ssig2;
        // Alt: cbet2 = StrictMath.hypot(csig2, salp0 * ssig2);
        cbet2 = StrictMath.hypot(_salp0, _calp0 * csig2);
        if (cbet2 == 0)
        ...
        if ((outmask & GeodesicMask.LONGITUDE) != 0) {
            ...
            double somg2 = _salp0 * ssig2, comg2 = csig2, // No need to normalize
                    E = StrictMath.copySign(1, _salp0); // east or west going?
            // omg12 = omg2 - omg1
            double omg12 = ((outmask & GeodesicMask.LONG_UNROLL) != 0)
                    ? E * (sig12 - (StrictMath.atan2(ssig2, csig2) - StrictMath.atan2(_ssig1, _csig1))
                            + (StrictMath.atan2(E * somg2, comg2) - StrictMath.atan2(E * _somg1, _comg1)))
                    : StrictMath.atan2(somg2 * _comg1 - comg2 * _somg1, comg2 * _comg1 + somg2 * _somg1);
            double lam12 = omg12 + _A3c * (sig12 + (Geodesic.SinCosSeries(true, ssig2, csig2, _C3a) - _B31));
            double lon12 = CustomStrictMath.toDegrees(lam12);
        ...
        if ((outmask & (GeodesicMask.REDUCEDLENGTH | GeodesicMask.GEODESICSCALE)) != 0) {
            double B22 = Geodesic.SinCosSeries(true, ssig2, csig2, _C2a), AB2 = (1 + _A2m1) * (B22 - _B21),
        ...
        if ((outmask & GeodesicMask.AREA) != 0) {
            ...
            if (_calp0 == 0 || _salp0 == 0) {
                ...
                salp12 = _calp0 * _salp0 * (csig12 <= 0 ? _csig1 * (1 - csig12) + ssig12 * _ssig1
                        : ssig12 * (_csig1 * ssig12 / (1 + csig12) + _ssig1));
                calp12 = GeoMath.sq(_salp0) + GeoMath.sq(_calp0) * _csig1 * csig2;
            }
            r.S12 = _c2 * StrictMath.atan2(salp12, calp12) + _A4 * (B42 - _B41);
        }
     ...
     *
     *            This is only useful if the GeodesicLine object has been
     *            constructed with <i>caps</i> |= {@link GeodesicMask#DISTANCE_IN}.
     **********************************************************************/
     ...
     *            negative.
     *
     *            The distance <i>s13</i> is only set if the GeodesicLine object has
     *            been constructed with <i>caps</i> |=
     *            {@link GeodesicMask#DISTANCE}.
     **********************************************************************/
     ...
     * @param arcmode
     *            boolean flag determining the meaning of the second parameter; if
     *            <i>arcmode</i> is false, then the GeodesicLine object must have
     *            been constructed with <i>caps</i> |=
     *            {@link GeodesicMask#DISTANCE_IN}.
     * @param s13_a13
     *            if <i>arcmode</i> is false, this is the distance from point 1 to
     *            point 3 (meters); otherwise it is the arc length from point 1 to
    ...
    /**
     * @return pair of sine and cosine of <i>azi1</i> the azimuth (degrees) of the
     *         geodesic line at point 1.
     **********************************************************************/
    public Pair AzimuthCosines() {
        return new Pair(Init() ? _salp1 : Double.NaN, Init() ? _calp1 : Double.NaN);
    }

    /**
     * @return <i>azi0</i> the azimuth (degrees) of the geodesic line as it crosses
    ...
    public Pair EquatorialAzimuthCosines() {
        return new Pair(Init() ? _salp0 : Double.NaN, Init() ? _calp0 : Double.NaN);
    }

    /**
     * @return <i>a1</i> the arc length (degrees) between the northward equatorial
    ...
    /**
     * @return <i>f</i> the flattening of the ellipsoid. This is the value inherited
     ...
     *
     * @param arcmode
     *            boolean flag determining the meaning of returned value.
     * @return <i>s13</i> if <i>arcmode</i> is false; <i>a13</i> if <i>arcmode</i>
     *         is true.
     **********************************************************************/
